...
const Route = method => `api/PCM/PurchaseTxn/${method}`

export const purchaseService = defineStore('purchaseService', {
  state: () => ({
  ...
  actions: {
    ...
    async update(model) {
      try {
        ...
        message.success('Compra actualizada con éxito.')
        return Data
      } catch (err) {
        message.error(err)
        return null
    ...
    async anular(txnId) {
      try {
        ...
        message.success('Compra anulada con éxito.')
        return Data
      } catch (err) {
        message.error(err)
        return null
