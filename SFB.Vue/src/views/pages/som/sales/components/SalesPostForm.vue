<template>
  <dialog-body v-model="showModal" title="Vender" fullscreen hide-actions class-body="bg-containerBg pt-2 px-3">
      ...
      <v-col cols="12" md="8" class="pa-0 pr-3 h-100">
        ...
        <header-bar class-card="pa-3" class="mb-2">
          ...
          <div class="d-flex align-center text-grey-darken-1 gap-4">
            ...
            <ui-icon name="ThunderboltOutlined" size="20" style="cursor: pointer;" class="mx-2" title="Quick Actions" />
          ...
          <v-col v-for="prod in products" :key="prod.NroProduct" cols="6" sm="4" md="4" lg="3">
            <v-card flat
              ...
              <div class="d-flex justify-space-between align-start mb-2">
                <div class="font-weight-bold text-subtitle-2 text-truncate mr-2">{{ prod.Name }}</div>
                <v-chip size="x-small" :color="prod.Stock > 0 ? 'success' : 'error'" label
                  class="font-weight-bold flex-shrink-0">
                  {{ prod.Stock }} {{ prod.Presentation?.Code }}
                </v-chip>
              </div>
        ...
        <v-card class="h-100 pa-4 bg-white d-flex flex-column" elevation="0" variant="outlined"
          ...
          <div class="mt-4 pt-4 border-t">
            ...
            <div class="d-flex gap-2">
              <v-btn variant="outlined" class="rounded-lg border-grey mr-1" height="50" width="50" color="grey-darken-1"
                @click="onCancel">
